...
//	public static final String DAUM_MAPS_LOCAL_KEYWORD_SEARCH_API_FORMAT = "https://apis.daum.net/local/v1/search/keyword.json?query=%s&location=%f,%f&radius=%d&page=%d&apikey=%s";
    public static final String DAUM_MAPS_LOCAL_KEYWORD_SEARCH_API_FORMAT = "https://apis.daum.net/local/v1/search/keyword.json?query=%s&location=%f,%f&radius=%d&page=%d&apikey=%s&sort=%d";
    ...
    private String buildKeywordSearchApiUrlString(String query, double latitude, double longitude, int radius, int page, String apikey) {
        ...
        // ye : 2 = 거리
        return String.format(DAUM_MAPS_LOCAL_KEYWORD_SEARCH_API_FORMAT, encodedQuery, latitude, longitude, radius, page, apikey, 2);
    }
    ...
    private List<Item> parse(String jsonString) {
        ...
        try {
            ...
            for (int i = 0; i < objects.length(); i++) {
                ...
                String[] category = item.category.split(" > ");
                if (category[0].trim().equals("음식점")) {
                    itemList.add(item);
                }
                Log.i("Searcher", item.title + ", " + item.phone + ", " + item.category);
            }
        } catch (Exception e) {
            e.printStackTrace();
            return null;
